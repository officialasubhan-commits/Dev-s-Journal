...
import { saveCourseSettings } from "./actions";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { CheckCircle, GraduationCap, AlertCircle } from "lucide-react";

export function CourseSettingsForm({ settings }: { settings: SiteSettings }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await saveCourseSettings(formData);
      if (res?.error) {
        setToast({ type: "error", text: res.error });
      } else {
        setToast({ type: "success", text: res?.success || "Course settings saved successfully!" });
        setTimeout(() => setToast(null), 4000);
      }
    } catch (error: any) {
      console.error(error);
      setToast({ type: "error", text: error?.message || "Failed to save course settings." });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <Card id="course" className="border-[var(--border-color)] overflow-hidden relative">
      {toast && (
      ...
      <CardHeader className="bg-[var(--background)]/50 border-b border-[var(--border-color)]">
        <CardTitle className="flex items-center gap-2 text-xl font-heading"><GraduationCap className="w-5 h-5 text-[var(--primary)]" /> Course Settings</CardTitle>
        <CardDescription>Default values for learning courses.</CardDescription>
      </CardHeader>
        ...
        <form onSubmit={handleSubmit} className={sectionCls}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input name="defaultInstructor" defaultValue={settings.defaultInstructor} required className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Default Currency</label>
              <input name="defaultCurrency" defaultValue={settings.defaultCurrency} required className={inputCls} />
            </div>
          </div>
          <div>
            <label className={labelCls}>Default Language</label>
            <select name="defaultLanguage" defaultValue={settings.defaultCourseLanguage} className={inputCls}>
