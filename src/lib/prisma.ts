...
const globalForPrisma = globalThis as unknown as {
  ...
  __prisma_pool: Pool | undefined
  __prisma_adapter: PrismaPg | undefined
}
...
const pool = globalForPrisma.__prisma_pool ?? new Pool({ connectionString })
const adapter = globalForPrisma.__prisma_adapter ?? new PrismaPg(pool)
const prisma = globalForPrisma.__prisma_v2 ?? new PrismaClient({ adapter })

if (process.env.NODE_ENV !== 'production') {
  globalForPrisma.__prisma_v2 = prisma
  globalForPrisma.__prisma_pool = pool
  globalForPrisma.__prisma_adapter = adapter
}
